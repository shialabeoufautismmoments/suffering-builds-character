...
UI.renderers.referrals = function (el) {
  ...
  el.innerHTML = `<div class="page-head"><div><div class="kicker">CLIENT GROWTH</div><h1>Referrals & Rewards</h1><div class="sub">Give every client a trackable link and follow each earned reward through fulfillment.</div></div></div>
    <div class="stat-tiles mb"><div class="stat-tile"><div class="label">Active codes</div><div class="value">${(DB.clients || []).length}</div></div><div class="stat-tile"><div class="label">Converted referrals</div><div class="value accent">${records.length}</div></div><div class="stat-tile"><div class="label">Pending rewards</div><div class="value">${pending}</div></div><div class="stat-tile"><div class="label">Fulfilled</div><div class="value">${fulfilled}</div></div></div>
    ...
    <div class="card mb"><div class="card-head"><h2>Client referral links</h2><span class="muted small">Links open the coaching application with attribution attached.</span></div><div class="referral-code-grid">${(DB.clients || []).slice().sort((a,b) => a.name.localeCompare(b.name)).map(client => `<article class="referral-code-card"><div><b>${UI.escape(client.name)}</b><code>${UI.escape(client.referralCode)}</code></div><button class="btn btn-xs btn-primary" onclick="Referrals.copyLink('${UI.attr(client.id)}')">Copy link</button></article>`).join('') || '<p class="muted">Add clients to create referral codes.</p>'}</div></div>
